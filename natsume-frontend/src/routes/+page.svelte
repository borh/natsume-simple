<script lang="ts">
	...
	const filteredResultCount = derived(
	    ...
	    ([$d, $selectedCorpora]) => {
	        ...
	        return Object.values($d).reduce((total, collocates) => {
	            return total + collocates.filter(collocate => 
	                $selectedCorpora.includes(collocate.corpus) || 
	                (collocate.contributions && collocate.contributions.some(c => $selectedCorpora.includes(c.corpus)))
	            ).length;
	        }, 0);
	    }
	);
	...
	function renderContributions(
		...
		return contributions
			...
			.map(({ corpus, frequency }) => {
				const width =
					((frequency * (useNormalization ? corpusNorm[corpus] : 1)) / getMax(collocates, useNormalization)) * 100;
			...
			if (collocate.contributions && collocate.contributions.length > 0) {
				...
				collocate.contributions.forEach(
					({ corpus, frequency }: { corpus: string; frequency: number }) => {
						distribution[corpus] = (distribution[corpus] || 0) + 
	...
	function ParticleHeader({
		...
		console.log('ParticleHeader result:', { particle, distribution, particleTotal, distributionWithOffsets });
		return {
			particle,
			distribution,
			particleTotal,
	...
	async function updateDerivedData() {
		const derivedData = computeDerivedData($results, $useNormalization, $selectedCorpora, $combinedSearch);
		d.set(derivedData);
	...
	async function getNPVs(): Promise<void> {
		try {
			isLoading.set(true);
			...
			searchElapsedTime.set((endTime - startTime) / 1000); // Convert to seconds
			results.set(data as Result[]);
			resultCount.set(data.length);
			lastSearchedNoun.set($noun); // Update the last searched noun
	...
	function tooltipAction(node: HTMLLIElement, { getTooltipData, useNormalization }: { getTooltipData: () => Record<string, number>, useNormalization: boolean }) {
	...
	function updateScrollButtonsVisibility() {
		if (mainScrollContainer) {
			...
			if (leftButton && rightButton) {
				...
				const totalHeaderHeight = (topHeader?.clientHeight || 0) + (particlesHeader?.clientHeight || 0);
	...
	function handleClickOutside(event: MouseEvent) {
		...
		if (statsDropdown && !statsDropdown.contains(event.target as Node) && !statsButton?.contains(event.target as Node)) {
			statsDropdownOpen = false;
		}
		if (optionsDropdown && !optionsDropdown.contains(event.target as Node) && !optionsButton?.contains(event.target as Node)) {
...
<div class="flex flex-col min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
    ...
    <header class="fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-2 z-20">
...
<div class="relative inline-block text-left">
    ...
    {#if statsDropdownOpen}
        <div id="stats-dropdown" class="origin-top-right absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-30">
            <div class="py-1">
                <!-- Corpus Sizes -->
                <div class="px-4 py-2">
                ...
                <div class="px-4 py-2 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <p class="dark:text-gray-300">Search time: {$searchElapsedTime.toFixed(2)} seconds</p>
                </div>
            </div>
        </div>
    {/if}
    ...
    {#if optionsDropdownOpen}
        <div id="options-dropdown" class="origin-top-right absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-30">
            <div class="py-1">
                <!-- Existing Options -->
                <div class="px-4 py-2">
                    ...
                    <div class="mt-2">
                        <span class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Corpus Selection</span>
    ...
    <header class="sticky top-12 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 z-10">
        <div class="container mx-auto p-0 overflow-x-auto hide-scrollbar" bind:this={headerScrollContainer} on:scroll={syncScroll}>
                            ...
                            <div style="width: {columnWidth}px; flex-shrink: 0;">
                                ...
                                <div class="h-1 w-full relative">
                                    {#each ParticleHeader({
                                        particle,
                                        collocates,
                                        maxFrequency: getMax(collocates, $useNormalization),
                                        useNormalization: $useNormalization,
                                        corpusNorm,
                                        totalFrequency: collocates.reduce((sum, c) => sum + c.frequency, 0),
                                        selectedCorpora: $selectedCorpora
                                    }).distributionWithOffsets as { corpus, width, xOffset }}
                                        <div 
                                            class="absolute h-full" 
                                            style="left: {xOffset}%; width: {width}%; background-color: {getSolidColor(corpus)};"
                                        ></div>
                                    {/each}
                                </div>
                            </div>
    ...
    <main class="flex-1 overflow-y-auto pt-12">
        <div class="container mx-auto p-0">
            <div class="relative">
                ...
                <div class="overflow-x-auto" bind:this={mainScrollContainer} on:scroll={syncScroll} style="width: 100%; max-width: 100vw;">
                    <div class="flex" style="gap: {columnSpacing}px;">
                        {#if $d && Object.keys($d).length > 0}
                            {#each Object.entries($d) as [particle, collocates]}
                                {#if collocates && collocates.length > 0}
                        ...
                        {:else}
                            <p class="text-center text-gray-600 mt-4">
                                No results found for "{$noun}". Try a different search term.
                            </p>
                        {/if}
                    </div>
                </div>
...
<div id="tooltip" class="hidden absolute bg-gray-700 dark:bg-gray-200 text-white dark:text-gray-800 text-sm p-2 rounded"></div>
